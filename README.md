...
1. 访问 [DashScope 控制台](https://dashscope.console.aliyun.com/) 注册/登录阿里云账号
2. 开通 Qwen 模型服务，在 **API-KEY 管理** 页面生成一个 Key
...
# DashScope 兼容 OpenAI 接口配置（请替换为你的真实 Key）
DASHSCOPE_API_KEY=sk-请替换为你的Key
DASHSCOPE_BASE_URL=https://ws-veyj3t9t0e5jiog7.cn-beijing.maas.aliyuncs.com/compatible-mode/v1
DASHSCOPE_MODEL=qwen3-coder-next
...
> 注意：`DASHSCOPE_API_KEY` 需要替换为你自己的真实 Key；如需更换模型（如 `qwen3-coder-next`），修改 `.env` 后重新启动服务即可。
